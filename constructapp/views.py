...
import re
from .models import UserDetails,Equipment,Requests,Requests_Apply
...
def searchpage(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
           ...
           accep=Requests_Apply.objects.filter()
           eq = Equipment.objects.filter(equipmentname=name).order_by('old')
           return render(request,'searchpage.html',{'name':eq,'accep':accep}) 
